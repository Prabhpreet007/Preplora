...
//     <p className="text-base text-left tracking-wide
//     text-neutral-500">
//       {tab.question}
//     </p>
...
export const QuestionSection = ({ questions }: { questions: { question: string; answer: string }[] }) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [isWebCam, setIsWebCam] = useState(false);
  ...
  return (
    <div className="w-full min-h-96 border rounded-md p-4">
      ...
      >
        {/* Tabs List */}
        <TabsList className="bg-transparent w-full flex flex-wrap items-center justify-start gap-4">
          {questions?.map((tab: { question: string; answer: string }, i: number) => (
            <TabsTrigger
              className={cn(
                "data-[state=active]:bg-emerald-200 data-[state=active]:shadow-md text-xs px-2"
              )}
